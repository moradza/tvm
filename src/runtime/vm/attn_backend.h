 ...
 public:
  ...
  void BeginForward(int depth, Tensor float_workspace_buffer, Tensor int_workspace_buffer,
    ...
    if (attn_kind == AttnKind::kMHA) {
      // enable_cuda_graph=true: FlashInfer sizes the plan for the reserved
      // maximum so replayed graphs stay valid across steps (resolves the
      // previous Todo(tvm-team); required for whole-step decode capture).
      plan_info_vec =
          plan_func_(float_workspace_buffer, int_workspace_buffer, page_locked_int_workspace_buffer,
                     ...
                     /*enable_cuda_graph=*/true, qk_head_dim, v_head_dim, causal,
